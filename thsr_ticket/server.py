...
class BookingRequest(BaseModel):
    ...
    personal_id: str
    phone: str
    seat_prefer: Optional[str] = (
        "radio16"  # radio16=None, radio17=Window, radio18=Aisle
    )
    ...
    try:
        ...
        if errs := check_error(book_resp.content):
            return BookingResponse(
                status="error",
                message="Booking failed at step 1 (Options)",
                data={"html": str(book_resp.content), "errors": errs},
            )
        ...
        train_resp, train_model = ConfirmTrainFlow(
            client, book_resp, record=updated_record
        ).run()
        if errs := check_error(train_resp.content):
            return BookingResponse(
                status="error",
                message="Booking failed at step 2 (Train)",
                data={"html": str(train_resp.content), "errors": errs},
            )

        # Step 3: Confirm Ticket
        ticket_resp, ticket_model = ConfirmTicketFlow(
            client, train_resp, updated_record
        ).run()
        if errs := check_error(ticket_resp.content):
            return BookingResponse(
                status="error",
                message="Booking failed at step 3 (Ticket)",
                data={"html": str(ticket_resp.content), "errors": errs},
            )
